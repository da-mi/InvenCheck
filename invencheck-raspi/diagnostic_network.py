...
def run_cmd(cmd):
    try:
        return subprocess.check_output(cmd, shell=True, timeout=5).decode(errors='ignore').strip()
...
def check_wifi_status():
    if IS_LINUX:
        ...
        ip = ip.split()[0] if ip else "NO IP"
    elif IS_WINDOWS:
        ssid = None
        try:
...
def timed_ping_overhead(host):
    ...
    try:
        if IS_LINUX:
            ...
            subprocess.check_call(["ping", "-n", "2", "-w", "2000", host], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        else:
            return False, None
...
def report(show_full=False):
    ...
    print(f"{'GW':<8}{'DNS':<8}{'8.8.8.8':<8}{'Web':<8}{'Supabase':<12}Status")
    print(f"{format_time(gw_time):<8}{format_time(dns_time):<8}{format_time(ext_time):<8}{format_time(web_time):<8}{format_time(supa_time):<12}", end="")
